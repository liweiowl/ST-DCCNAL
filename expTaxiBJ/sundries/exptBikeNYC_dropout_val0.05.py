...
from models.CrossLayer_model_dense_dropout import myCrossLayer
...
def build_model(window_len, lr):
    ...
    model.summary()
    plot_model(model, to_file='dropout_reg_model.png', show_shapes=True)
    return model
...
earlystoping = EarlyStopping(monitor='val_root_mean_square_error', patience=50, mode='min')
checkpoint = ModelCheckpoint(filepath=model_name, monitor='val_root_mean_square_error',
                             verbose=1, save_best_only=True, mode='min', period=1)# checkpoint2 = ModelCheckpoint(filepath='./others/model_best_2.h5', monitor='val_root_mean_square_error', verbose=1, save_best_only=True,mode='min', period=1)
hist = model.fit(x=xtr, y=ytr, batch_size=batch_size, epochs=epochs, validation_split=0.05, shuffle=True,
