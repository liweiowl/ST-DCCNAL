...
def dense_conv3D(nb_filter=64, nb_row=5, nb_col=5, padding='same', bn=False, nb_layers=3,dropout_rate=0.3):
    """
    ...
    def f2(input):
        ...
        for i in range(nb_layers):
            tmp = basic_conv2d(nb_filter=nb_filter, nb_row=nb_row, nb_col=nb_col, padding=padding, bn=bn,dropout_rate=dropout_rate)(tmp)
...
def myCrossLayer(nb_flow=2, map_height=16, map_width=8, nb_layers=3, window_len=12, nb_filter=64):
    ...
    for i in range(window_len):
        ...
        cnn_fea = dense_conv3D(nb_filter=nb_filter, nb_col=5, nb_row=5, padding='same', nb_layers=nb_layers)(inputs)
...
def build_model(lr=0.001):
    ...
    model.summary()
    plot_model(model, to_file='model_structure_png/my_hybrid_model.png', show_shapes=True)
