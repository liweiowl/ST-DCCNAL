...
def dense_conv3D(nb_filter=64, nb_row=5, nb_col=5, padding='same', bn=False, nb_layers=3, dropout_rate=0.5,
    ...
    def f2(input):
        ...
        tmp1 = cnn_out[-1]
        tmp2 = Reshape(([nb_filter * 32 * 32]))(tmp1)
...
def myCrossLayer(nb_flow=2, map_height=16, map_width=8, nb_layers=3, window_len=12, nb_filter=64,
    ...
    if external_dim != None and external_dim > 0:
        print('the external dim is', external_dim)
        for i in range(window_len):
            ...
            window_len_pic_fea.append(cnn_fea_flatten)
        for i in range(window_len):
            external_inputs = Input(shape=(external_dim,))
            main_inputs.append(external_inputs)
            embedding = Dense(nb_layers * 1024, activation='relu')(external_inputs)
