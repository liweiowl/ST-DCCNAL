...
def add_densenet(nb_flow=2, map_height=16, map_width=8):
    """
    ...
    def f3(data_in):
        ...
        main_output2 = Reshape(([6*3072]))(main_output1)
        main_output2 = Dense(units=nb_flow* map_height* map_width)(main_output2)
...
def myCrossLayer(nb_flow=2, map_height=16, map_width=8, nb_layers=3, window_len=12, nb_filter=64,
    ...
    if external_dim == None:
        for i in range(window_len):
            ...
            cnn_fea = Concatenate_conv3D(nb_filter=nb_filter, nb_col=filter_size, nb_row=filter_size, padding='same',
                                   nb_layers=nb_layers, dropout_rate=0.5)(inputs)
            # cnn_fea_flatten = Reshape(([nb_layers * 1024]))(cnn_fea)
            cnn_fea_flatten = Reshape(([3072]))(cnn_fea)
    ...
    outputs = add_densenet(nb_flow=nb_flow, map_height=map_height, map_width=map_width)(window_len_pic_fea)
